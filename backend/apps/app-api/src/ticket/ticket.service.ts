...
export class TicketsService {
  ...
  async create(dto: CreateTicketDto & { authorId: string }) {
    ...
    const ticket = await this.tenantPrisma.client.ticket.create({
      data: {
        // ticketNumber is generated by DB default (sequence)
        title: dto.title,
  ...
  async updateComment(ticketId: string, commentId: string, userId: string, body: string) {
    const comment = await this.tenantPrisma.client.comment.findUnique({ where: { id: commentId } })
    ...
    const updated = await this.tenantPrisma.client.comment.update({ where: { id: commentId }, data: { body } })
    const [enriched] = await this.userEnrichment.enrichEntities([updated], ['authorId'])
    return enriched
  }

  async deleteComment(ticketId: string, commentId: string, userId: string) {
    const comment = await this.tenantPrisma.client.comment.findUnique({ where: { id: commentId } })
    if (!comment || comment.ticketId !== ticketId) {
      throw new NotFoundException('Comment not found')
    }
    ...
    await this.tenantPrisma.client.comment.delete({ where: { id: commentId } })
    return { ok: true }
  }
