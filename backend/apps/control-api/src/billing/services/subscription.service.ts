...
export class SubscriptionService {
  ...
  async checkLimits(tenantId: string): Promise<UsageLimitsCheck> {
    try {
      ...
      // Récupérer les limites depuis les champs Plan Prisma
      const limits = {
        users:
          typeof (subscription.plan as any).maxUsers === 'number'
            ? Number((subscription.plan as any).maxUsers)
            : -1,
        tickets:
          typeof (subscription.plan as any).maxTickets === 'number'
            ? Number((subscription.plan as any).maxTickets)
            : -1,
        storage:
          typeof (subscription.plan as any).maxStorage === 'number'
            ? Number((subscription.plan as any).maxStorage)
            : -1,
        apiCalls:
          typeof (subscription.plan as any).maxApiCalls === 'number'
            ? Number((subscription.plan as any).maxApiCalls)
            : -1,
      };
  ...
  // ════════════════════════════════════════════════════
  // 👀 PRÉVISUALISATION CHANGEMENT DE PLAN
  // ════════════════════════════════════════════════════
  ...
  async previewPlanChange(
    ...
    const subscription = await this.getSubscriptionByTenant(tenantId);
    if (!subscription) {
      // Prévisualisation d'une première souscription
      const newPlan = await this.planService.getPlanById(newPlanId);
    ...
    const currentPrice =
      cycle === BillingCycle.MONTHLY
        ? Number((currentPlan as any).priceMonthly)
        : Number(
            (currentPlan as any).priceYearly
              ? (currentPlan as any).priceYearly
              : Number((currentPlan as any).priceMonthly) * 12,
          );

    const targetPlan = await this.planService.getPlanById(newPlanId);
    const newPrice =
      cycle === BillingCycle.MONTHLY
        ? Number(targetPlan.priceMonthly)
        : Number(targetPlan.priceYearly || targetPlan.priceMonthly * 12);

    // Estimation de prorata (simple): proportion du temps restant du cycle courant
    ...
    if (subscription.startDate && subscription.endDate) {
      ...
      prorationAmount = Math.round((newPrice - currentPrice) * ratio * 100) / 100;
    ...
    // Tentative de preview avancée via Stripe (si possible)
    try {
      if (subscription.stripeSubscriptionId) {
        ...
        // Récupérer le price cible depuis la table Plan (DB) pour éviter la dépendance à la config
        const dbPlan = await this.controlPrisma.plan.findUnique({
          where: { id: newPlanId },
          select: {
            id: true,
            name: true,
        ...
        const targetPriceId =
          (billingCycle || subscription.billingCycle) === BillingCycle.MONTHLY
            ? dbPlan?.stripePriceIdMonthly
            : dbPlan?.stripePriceIdYearly;

        if (targetPriceId) {
          const itemId = stripeSub.items.data[0]?.id;
          // Utilise retrieveUpcoming si disponible (cast any pour compat TS)
          const upcoming = await (stripe.invoices as any).retrieveUpcoming({
            subscription: stripeSub.id,
            subscription_items: [
              { id: itemId, price: targetPriceId },
            ],
            subscription_proration_date: Math.floor(Date.now() / 1000),
            expand: ['lines', 'customer'],
          });

          if (upcoming && typeof upcoming.total === 'number') {
            const total = (upcoming.total || 0) / 100;
            const subtotal = (upcoming.subtotal ?? total) / 100;
            const prorata = Math.max(0, total - (currentPrice || 0));
            ...
            return {
              ...
              newPlan: { id: targetPlan.id, name: targetPlan.name, price: newPrice },
              priceChange,
              prorationAmount: Math.round(prorata * 100) / 100,
              nextBillingDate,
              effectiveNow: true,
      ...
      this.logger.warn(
        `Stripe preview failed, falling back to local estimation: ${
