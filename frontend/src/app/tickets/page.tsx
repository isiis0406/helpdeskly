...
async function getTickets(filters: { q?: string; status?: string; priority?: string; assignedToId?: string; assignee?: string; page?: string; limit?: string }): Promise<{ items: Ticket[]; pagination?: Pagination }> {
...
export default async function TicketsListPage({ searchParams }: { searchParams?: Promise<{ q?: string; status?: string; priority?: string; assignedToId?: string; assignee?: string; page?: string; limit?: string }> }) {
  const sp = (await searchParams) || {};
  const tenantSlug = (await cookies()).get("tenantSlug")?.value;
  const { items: tickets, pagination } = tenantSlug ? await getTickets(sp) : { items: [], pagination: undefined };
          ...
          <>
            ...
            {pagination && (
              <div className="flex items-center justify-between text-sm mt-3">
                <Link href={{ pathname: '/tickets', query: { ...sp, page: String(Math.max(1, (pagination.page || 1) - 1)) } as any }} className={`px-3 h-9 rounded border border-border inline-flex items-center ${pagination.page <= 1 ? 'pointer-events-none opacity-50' : ''}`}>Précédent</Link>
                <div className="text-muted-foreground">Page {pagination.page} / {pagination.pages}</div>
                <Link href={{ pathname: '/tickets', query: { ...sp, page: String(Math.min(pagination.pages, (pagination.page || 1) + 1)) } as any }} className={`px-3 h-9 rounded border border-border inline-flex items-center ${pagination.page >= pagination.pages ? 'pointer-events-none opacity-50' : ''}`}>Suivant</Link>
              </div>
            )}
          </>
        )}
...
function BadgeStatus({ value }: { value: 'OPEN'|'IN_PROGRESS'|'RESOLVED'|'CLOSED' }) {
...
function BadgePriority({ value }: { value: 'LOW'|'MEDIUM'|'HIGH'|'URGENT' }) {
